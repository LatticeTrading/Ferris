use super::{
    ...
    Config, Mode, OrderBookSnapshot, ParseResult, TradeDeduper, TradeRow,
};
...
#[test]
fn parse_args_rejects_ohlcv_mode() {
    let args = vec!["ohlcv".to_string()];
    let err = parse_args(&args).expect_err("ohlcv mode should be rejected");
